...
def download_trade_data(filename, human_readable=False, verbose=True, period='2016', frequency='A', reporter='USA',
                        partner='all', product='total', tradeflow='exports', blob=False):
    """
    ...
    if sum('all' in inpt for inpt in [reporter, partner, period]) > 1:
        raise ValueError("Only one of the parameters 'reporter', 'partner' and 'period' "
    ...
    if len(dfs) > 0:
        ...
        if blob:
            blob_manager.create_blob_from_path(blob_manager.generate_blob_name(filename),
